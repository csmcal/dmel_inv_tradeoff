...
#Purpose: For easy running of the alignment and frequency analysis pipeline on the Pool Lab marula server,
#   with command output recording and nohup built in,
#   and specifically for the library prep variance controls 

#Run as: 
# ./var_ctrl_freq_pipe_marula_nohup.sh

# Generate log file:
TIMESTAMP=$(date +"%Y-%m-%d_%H-%M-%S")
LOG='nohup_var_ctrl_freq_pipe_'$TIMESTAMP'.log'
printf '\nGenerating log file:'
printf $LOG'\n'
...
printf 'An instance of var_ctrl_freq_pipe_marula_nohup.sh, running the inv alignment/frequency analysis pipeline on Pool lab servers\n' >> $LOG 2>&1
...
read_metadata_file='variance_controls/var_control_meta.csv'
...
intermediate_directory='variance_controls/aln_intermediates/'
aligned_output_directory='variance_controls/aligned_bam/'
amp_haps_output_directory='variance_controls/aln_hap_counts/'
...
inversion_frequency_output_directory='variance_controls/inv_freqs/'
